...
def train():
    ...
    rank0_print("🎬 Multi-Video GRPO Training Started!")
    rank0_print(f"📊 Model: {model_args.model_id}")
    rank0_print(f"🎥 Data: {data_args.data_path}")
    rank0_print(f"🏆 Reward Function: numerical_video_reward")
    ...
    # Validation checks for multi-video training
    ...
    # Handle LoRA layers for quantized models
    if training_args.bits in [4, 8]:
    ...
    # Create dataset (supports multi-video)
    dataset_module = make_grpo_data_module(model_id=model_args.model_id,
                                              processor=processor,
                                              data_args=data_args)

    # Load the custom numerical reward function
    rank0_print("🎯 Loading numerical video reward function...")
    reward_funcs = load_reward_funcs("src.train.reward_funcs", ["numerical_video_reward"])
    ...
    # Save model based on LoRA configuration
    if training_args.lora_enable:
    ...
    rank0_print("✅ Multi-Video GRPO Training Complete!")
